...
<script type="text/javascript" charset="utf-8">
	...
	function BeforeClick(event, treeId, treeNode) {
		if ($(".catalog-list").length != 1) {
			//文章管理页面
			if ($("#articleModal").css("display") != "block") {
				$(".article-list").load("sys/article_list?catalogId=" + treeId.id);
			} else {
				//发布文章
				$("#inputCatalog").val(treeId.id);
				$("span.selected-catalog").html(treeId.name);
			}
		} else {
			//栏目管理页面
			if ($(".catalog-list .form-edit-catalog").css("display") != "block") {
				$(".catalog-list").load(
						"sys/catalog_children?catalogId=" + treeId.id);
			} else {
				//添加栏目
				$("#inputCatalog").val(treeId.id);
				$("span.selected-catalog").html(treeId.name);
...
<div id="rMenu">
		<ul>
				<li id="m_add" onclick="addTreeNode();">增加栏目</li>
				<li id="m_del" onclick="removeTreeNode();">删除栏目</li>
				<!-- <li id="m_check" onclick="checkTreeNode(true);">Check节点</li>
				<li id="m_unCheck" onclick="checkTreeNode(false);">unCheck节点</li> -->
				<li id="m_reset" onclick="resetTree();">刷新栏目</li>
		</ul>
</div>
